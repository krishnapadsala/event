...
router
  .route("/addcategory")
  .post(verifyJWT, upload.single("URL"), postCategory);
router
  .route("/deletecategory/:id")
  .delete(upload.none(), verifyJWT, deleteCategory);
router.route("/showcategory").get(showCategory);
router.route("/showeventsbycategory").post(upload.none(), showEventsByCategory);
router
  .route("/updatecategory/:id")
  .post(verifyJWT, upload.single("URL"), updateCategory);
router
  .route("/updateevent/:id")
  .post(verifyJWT, upload.single("URL"), updateEvent);

router.route("/getalluser").get(upload.none(), verifyJWT, alluser);
router
  .route("/blockandunblockuser")
  .post(upload.none(), verifyJWT, blockandunblockuser);
